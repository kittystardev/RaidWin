...
export default function NFTPoolCard1vs1({
  title,
  banner,
  profile,
  content,
  ...
  const Play1vs1Game = async () => {
    try {
      const resp = await fetch(
        `${baseUrl}/gameState/${getCollectionMint(gamesState, true)}/2/true`
      );
      const obj = await resp.json();
      if (obj.msg === "created") {
        navigate(`/dogevscheems/${slug}?gameId=${obj.data.account}`);
  ...
  return (
    <>
      <div className="bg-yankees-blue hover:bg-rhino rounded-3xl">
        <div className="justify-center items-center  h-[160px] rounded-t-3xl overflow-hidden cursor-pointer">
          {location.pathname === "/games" ? (
            <Link to={`../games/${slug}`}>
              <img
                src={banner}
                className="rounded-t-3xl min-w-[100%] max-w-[100%] h-auto min-h-[160px] bg-no-repeat bg-contain"
              />
            </Link>
          ) : (
            <img
              ...
              className="rounded-t-3xl min-w-[100%] max-w-[100%] h-auto min-h-[160px] bg-no-repeat bg-contain image-hover"
        ...
        <div className="p-8 cursor-pointer">
          {location.pathname === "/games" ? (
            ...
            <div className="relative mt-4" onClick={() => Play1vs1Game()}>
              <div className="flex items-center gap-2 mt-8">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-5 h-5 text-space-gray-rgb"
                  ...
                  <div className="flex gap-2">
                    <div className="text-sm font-bold leading-8 tracking-wide">
                      <span className="text-light-green">
                        {getNumberOfJoined(gamesState, true)}
                      </span>
                      <span className="text-nouveau-main">/2</span>
                    </div>
